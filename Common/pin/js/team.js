$(function(){
	...
	$('.goods').on('click',function(){
		...
		var sku_infock_length = $('.goods.checked ').length;
		...
		/**
		var mul_op_pin_price = Array();
		var mul_op_dan_price = Array();
		**/
		var pin_type = $('#pin_type').val();
		//$('#pin_type').val('dan'); 'pin'
		var new_price = 0;
		if(pin_type == 'pin')
		{
			new_price = mul_op_pin_price[mult_option_ckids];
		} else{
			new_price = mul_op_dan_price[mult_option_ckids];
		}
		//¥ 0.01 sku-price-depends
		$('#sku-price-depends').html('¥ '+new_price);
	...
	$('#tuan_more_btn').click(function(){
		...
		if(has_option){
			...
			var sku_infock_length = $('.goods.checked ').length;
			if(sku_infock_length != undefined && sku_infock_length == sku_info_length)
			{
				var mult_option_ckids = get_option_check_ids();
				var pin_type = $('#pin_type').val();
				var new_price = 0;
