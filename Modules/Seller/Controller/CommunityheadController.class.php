...
class CommunityheadController extends CommonController {
	...
	public function index(){
		...
		$all_sell_count = M('lionfish_comshop_goods')->where( array('is_all_sale' => 1,'type' => 'normal') )->count();
		...
		foreach( $list as $key => $val )
		{
			...
			//团长商品
			$head_goods_count_arr = M()->query("select g.id from ".C('DB_PREFIX')."lionfish_community_head_goods as hg ,".C('DB_PREFIX')."lionfish_comshop_good_common as gc ,".C('DB_PREFIX')."lionfish_comshop_goods as g 
	...
	public function distribulist()
	{
        ...
        $sql = 'SELECT * FROM ' . C('DB_PREFIX'). "lionfish_community_head_tixian_order o left join " .C('DB_PREFIX')."lionfish_comshop_member  m               
        ...
        $total = M()->query('SELECT count(1) as count FROM ' . C('DB_PREFIX').
            'lionfish_community_head_tixian_order o left join '.C('DB_PREFIX').'lionfish_comshop_member m on o.member_id = m.member_id WHERE 1 ' . $condition);
	...
	function community_qrcode(){
		...
		$community_info = M('lionfish_community_head')->where( "id=".$community_id )->find();
	...
	function agent_check_first(){
		...
		if ( IS_POST ) {
			...
			$member = M('lionfish_community_head')->field('id,member_id,state')->where( array('id' => $id) )->find();
		
			M('lionfish_community_head')->where( array('id' => $id ) )->save( array('state' => 1,'enable' => 1, 'apptime' => $time) );
			$community_model = D('Seller/Community');
			//检测是否存在账户，没有就新建
			//TODO....sendmsg  发送成为团长的信息
			$community_model->send_head_success_msg($member['id']);
	...
	function head_mobile(){
			...
			$headid = M('lionfish_comshop_member')->field('member_id,telephone')->select();
			foreach($headid as $val){
				if($val['telephone']){
					$head_mobile = substr($val['telephone'],0,7).'7878';

					M('lionfish_comshop_member')->where( array('member_id' => $val['member_id'] ) )->save( array('telephone' => $head_mobile) );
				}
