...
class OrderController extends CommonController{
	...
	protected function _initialize(){
     ...
     public function index(){
		...
		if(SELLERUID != 1)
		{
			...
			$perms_arr = M('lionfish_comshop_perm_role')->where( array('id' => $seller_info['s_role_id']) )->find();
		...
		if (IS_POST) {
			...
			if (!(empty($id))) {
				...
				if ($changead) {
					...
					$province_info = M('lionfish_comshop_area')->where("name like '%{$province}%' ")->find();
					...
					$city_info = M('lionfish_comshop_area')->where("name like '%{$city}%'")->find();
					...
					$country_info = M('lionfish_comshop_area')->where( "name like '%{$area}%' " )->find();
	...
	public function oprefund_goods_do()
	{
		...
		**/
		...
		if ( IS_POST ) {
			...
			//已退商品数量
			$refund_total_quantity = M('lionfish_comshop_order_goods_refund')->where( "order_id={$id} and order_goods_id={$order_goods_id} " )->sum('quantity');
			...
			else if( $is_back_sellcount == 1 && $refund_quantity > $total_quantity )
			{
				...
				if( $res['code'] == 0 )
				{
					...
					if( $new_total_quantity <=0 )
					{
						...
						$order_goods_list = M('lionfish_comshop_order_goods')->where( array('order_id' => $id ) )->select();
						$is_all_refund = true;
						...
						$or_gd_info = M('lionfish_comshop_order_goods')->where( array('order_goods_id' => $order_goods_id ) )->find();
	...
	public function oprefund_do()
	{
		...
		$has_refund_buy_score_total = M('lionfish_comshop_member_integral_flow')->where(array('type' => 'refundorder' , 'order_id' => $id ))->sum('score');
		...
		if( IS_POST )
		{
			...
			//已经退款运费
			$refund_shipping_fare_amount = M('lionfish_comshop_order_goods_refund')->where( array('order_id' =>$id ) )->sum('refund_shipping_fare');
			...
			else if( $refund_money < 0 )
			{
				...
				if( $order_info['order_status_id'] != 7 )
				{
					...
					if( $res['code'] == 0 )
					{
						...
						//integral
						$order_info = M('lionfish_comshop_order')->where( array('order_id' => $id ) )->find();
						...
						if($is_refund_shippingfare == 1)
						{
							if( $order_info['type'] == 'integral' ){
								...
								$refund_shipping_fare_amount2 = M('lionfish_comshop_order_goods_refund')->where( array('order_id' =>$id ) )->sum('refund_shipping_fare');
	...
	**/
	public function order_print_dan()
	{
		...
		$total = M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id) )->sum('total');
	...
	public function opsend_tuanz()
	{
		...
		$order_num_alias = M('lionfish_comshop_order')->where(  array('order_id' => $item['order_id'] ) )->find();
		...
		**/
        ...
        $columns = array(
            ...
            array('title' => '会员手机号', 'field' => 'telephone', 'width' => 12, 'sort' => 0, 'is_check' => 1),
				...
				array('title' => '供应商名称/类型', 'field' => 'supply_name', 'width' => 12, 'sort' => 0, 'is_check' => 1),
	...
	public function detail()
	{
		...
		foreach($order_goods as $key => $value)
		{
				...
				foreach( $head_commission_order_info  as  &$vv)
				{
					...
					if($vv['type'] == 'orderbuy' && $vv['add_shipping_fare'] > 0){
						$refund_shipping_sql = "SELECT sum(refund_shipping_fare) as refund_shipping_fare "
								. " FROM ".C('DB_PREFIX')."lionfish_comshop_order_goods_refund "
			...
			{
				$refund_info = M('lionfish_comshop_order_refund')->where( array('order_id' => $id,'order_goods_id' => $value['order_goods_id'] ) )->order('ref_id desc')->find();

				$value['refund_info'] = $refund_info;

				$goods_refund_shipping_fare = M('lionfish_comshop_order_goods_refund')->where( array('order_id' => $id,'order_goods_id' => $value['order_goods_id'] ) )->sum('refund_shipping_fare');
			...
			$has_refund_money = M('lionfish_comshop_order_goods')->where( array('order_id' => $id,'order_goods_id' => $value['order_goods_id'] ) )->find();
			...
			if($item['delivery'] == 'hexiao'){
			    $value['hx_info'] = M('lionfish_comshop_order_goods_saleshexiao')->where( array('order_id' => $id,'order_goods_id' => $value['order_goods_id'] ) )->find();
			    if(!empty($value['hx_info'])){
					...
					$record_count = M('lionfish_comshop_order_goods_saleshexiao_record')->where(array('order_id' => $id,'order_goods_id' => $value['order_goods_id'] ) )->count();
		...
		{
			$item['orderdistribution_order'] = M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $item['order_id'] ) )->find();
			if( !empty($item['orderdistribution_order']) && $item['orderdistribution_order'] > 0 )
			{
				if($item['orderdistribution_order']['orderdistribution_id'] > 0){
					$orderdistribution = M('lionfish_comshop_orderdistribution')->where( array('id' => $item['orderdistribution_order']['orderdistribution_id']  ) )->find();
	...
	public function opreceive()
	{
		...
		if( $item['delivery'] == 'localtown_delivery' )
		{
			M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $item['order_id']) )->save( array('state' => 4) );
		...
		if (IS_POST) {
			...
			if($cansub == 1)
			{
				...
				if( $order_info['type'] == 'integral' || $order_info['type'] == 'normal' )
				{
					...
					{
					    $order_goods =  M('lionfish_comshop_order_goods')->where( array('order_goods_id' =>$refund_info['order_goods_id'] ) )->find();
					   
					    M('lionfish_comshop_order_goods')->where( array('order_goods_id' =>$refund_info['order_goods_id'] ) )->save( array('shipping_fare' => $order_refund['ref_shipping_fare'] ) );
					}
				}
				...
				if( $order_info['type'] == 'integral' || $order_info['type'] == 'normal')
				{
					...
					if( !empty($refund_info['order_goods_id']) && $refund_info['order_goods_id'] > 0 )
					{
					    M('lionfish_comshop_order_goods')->where( array('order_goods_id' =>$refund_info['order_goods_id'] ) )->save( array('shipping_fare' => $order_goods['shipping_fare'] ) );
					}
	...
	public function do_order_quene()
	{
			...
			if( $type == 'mult' && $order_info['delivery'] == 'express' )
			{
				...
				if($order_info['delivery'] != 'express')
				{
					...
					/*if($order_info['delivery'] == 'localtown_delivery'){
						...
						M('lionfish_comshop_order')->where( array('order_id' => $order_info['order_id']) )->save( array('order_status_id' => 4,'express_time' => time(),  'express_tuanz_time' => time()) );
						...
						M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_info['order_id']) )->save( array('delivery_type' => 1) );
		...
		{
			/*if($order_info['delivery'] == 'localtown_delivery'){
				M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_info['order_id']) )->save( array('state' => 4,'orderdistribution_id'=> -1) );

				D('Seller/Order')->receive_order($order_info['order_id']);

				M('lionfish_comshop_order_history')->where( array('order_id' => $order_info['order_id'],'order_status_id' => 6) )->save( array( 'comment' => '系统自动收货，等待结算佣金') );
	...
	public function kdn_print_log_list(){
		$log_list = M('lionfish_comshop_order_kdniao_print_log')->order('id desc')->limit(5)->select();
	...
	public function kdniao_print_orders(){
		...
		if($print_type == "select"){
			$order_list = M('lionfish_comshop_order')->field('order_id,is_kdn_print')->where("order_status_id = 1 and delivery = 'express' and order_id in (".$order_id.")")->select();
		}else if($print_type == "all"){
			$order_list = M('lionfish_comshop_order')->field('order_id,is_kdn_print')->where("order_status_id = 1 and delivery = 'express' ")->select();
		}
		...
		$kdn_id = M('lionfish_comshop_order_kdniao_print_log')->add( $kd_data );
		$succ_count = 0;
		$fail_count = 0;
		$fail_result = array();
		$kdNiao = new \Lib\KdApiEOrder();
		...
		$kdn_data['status'] = 1;
		M('lionfish_comshop_order_kdniao_print_log')->where(array('id' => $kdn_id))->save($kdn_data);
	...
	public function kdn_info(){
		$this->order_id = I('request.order_id','');
		$kdn_info = M('lionfish_comshop_order_kdniao_info')->where( array('order_id' => $this->order_id,'status'=>1) )->find();
	...
	public function kdn_orders_info(){
		...
		$kdn_list_info = M('lionfish_comshop_order_kdniao_info')->where("order_id in (".$this->order_ids.") and status = 1")->select();
	...
	public function kdn_send(){
		...
		if(!empty($order_id)){
			$item = M('lionfish_comshop_order')->where( array('order_id' => $order_id) )->find();
			if (empty($item)) {
				show_json(0, '未找到订单!');
			}
			$order_info = M('lionfish_comshop_order')->where( array('order_id' => $order_id) )->find();
	...
	public function change_order(){
		...
		if( IS_POST )
		{
            ...
            foreach($order_goods_ids as $k=>$order_goods_id){
                $order_goods_info = M('lionfish_comshop_order_goods')->where( array('order_id' =>$id,'order_goods_id' =>$order_goods_id  ) )->find();
			...
			foreach($order_goods_ids as $k=>$order_goods_id){
				...
				if(!empty($change_prices[$k])){
					...
					M('lionfish_comshop_order_goods')->where( array('order_id' =>$id,'order_goods_id' => $order_goods_id  ) )->save( $update_data);
				}
			}
			$order_info = M('lionfish_comshop_order')->where( array('order_id' =>$id) )->find();
			...
			M('lionfish_comshop_order')->where( array('order_id' =>$id) )->save($order_data);
	...
	public function thirth_delivery_order(){
		...
		$sql = "select og.goods_id,og.name as goods_name,og.quantity,og.price,og.total,og.rela_goodsoption_valueid,g.weight as goods_weight  from "
				. C('DB_PREFIX')."lionfish_comshop_order_goods as og left join  ".C('DB_PREFIX')."lionfish_comshop_goods as g on og.goods_id=g.id "
		...
		foreach($goods_list as $k=>$v){
			...
			if(!empty($v['rela_goodsoption_valueid'])){
				$goods_option_mult_value = M('lionfish_comshop_goods_option_item_value')->where( array('option_item_ids' => $v['rela_goodsoption_valueid'],'goods_id' => $v['goods_id']) )->find();
		...
		//收货人经纬度
		$order_distribution_info = M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_id ) )->find();
		//收货人地址纬度
		...
		}else if( $data_type == 'make' )
		{
			...
			//保存蜂鸟即配商户订单号
			M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_id ) )->save(['order_code'=>$order_code]);
	...
	public function thirth_cancel_delivery_order(){
		...
		$orderdistribution_info = M('lionfish_comshop_orderdistribution_order')->where( array('order_id' =>$order_id) )->find();
	...
	public function third_delivery_log_list(){
		...
		$log_list = M('lionfish_comshop_orderdistribution_thirth_log')->where( array('order_id' =>$order_id,'third_distribution_type'=>$distribution_type) )->order('addtime desc')->select();
	...
	public function order_goods_hexiao(){
	    ...
	    $order_hexiao_info = M('lionfish_comshop_order_goods_saleshexiao')->where( array('order_id' => $id, 'order_goods_id' => $order_goods_id) )->find();
	...
	public function get_goods_saleshexiao(){
	    ...
	    $order_goods_info = M('lionfish_comshop_order_goods')->where(array('order_goods_id'=>$order_hexiao_info['order_goods_id']))->field('goods_id,name as goods_name,goods_images,quantity')->find();
	...
	public function hexiao_goods(){
	    ...
	    $order_hexiao_info = M('lionfish_comshop_order_goods_saleshexiao')->where( array('id' => $hx_id) )->find();
	    if (empty($order_hexiao_info)) {
	        show_json(0, '未找到核销订单!');
	    }
	...
	public function hexiao_goods_assign_salesroom(){
	    ...
	    $order_goods_info = M('lionfish_comshop_order_goods')->where(array('order_goods_id'=>$order_goods_id))->field('goods_id,name as goods_name,goods_images,quantity')->find();
