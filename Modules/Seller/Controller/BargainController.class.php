...
class BargainController extends CommonController{
	...
	protected function _initialize(){
		parent::_initialize();
		$this->breadcrumb1='砍价管理';
		$this->breadcrumb2='砍价商品管理';
	...
	public function index(){
	        ...
	        $data=$model->show_bargain_page($search);
	...
	public function edit_goods()
	{
	    ...
	    $pin_goods = M('bargain_goods')->where( array('id' => $id) )->find();
	    //goods_id
	...
	{
		$id = I('get.id');
		$pin_goods_info = M('bargain_goods')->field('goods_id')->where( array('id' => $id) )->find();
		...
		M('bargain_goods')->where( array('id' => $id) )->delete();
	...
	public function modify_pin()
	{
	    ...
	    foreach($data['goods_ids_arr'] as $goods_data)
	    {
	        ...
	        $pin_goods['bargain_price'] = $goods_info[1];
	        $pin_goods['bargain_count'] = $goods_info[2];
	        
			$pin_goods['begin_time'] = strtotime( $goods_info[7]);
			$pin_goods['end_time'] =  strtotime( $goods_info[8]) ;
			$pin_goods['hour'] =  floatval( $goods_info[9]) ;
			
			$pin_goods_info =  M('bargain_goods')->where( array('goods_id' => $goods_info[0]) )->find();
			
	        M('bargain_goods')->where( array('goods_id' => $goods_info[0]) )->save($pin_goods);
	...
	public function sub_pin()
	{
		...
		array(3) {
		  ...
		  array(1) {
			[0]=>
			string(171) "88,1,2,100_pin_price:2@@101_pin_price:3@@102_pin_price:0.00@@103_pin_price:0.00,,100_quantity:11109@@101_quantity:11111@@102_quantity:11111@@103_quantity:11111,1,undefined"
		  }
		  ["begin_time"]=>
		  string(18) "2018/06/01 0:00:00"
		  ["end_time"]=>
		  string(18) "2018/06/02 0:00:00"
		}
		string(18) "2018/06/01 0:00:00"
		string(18) "2018/06/02 0:00:00"
		...
		**/
	    ...
	    foreach($data['goods_ids_arr'] as $goods_data)
	    {
			...
			//避免二次开团
			M('bargain_goods')->where( array('goods_id' => $goods_info[0]) )->delete();
			/**
				goods_id
				customer_id
				begin_time
			...
			**/
	        ...
	        $pin_goods['bargain_price'] = $goods_info[1];
	        $pin_goods['bargain_count'] = $goods_info[2];
	        $pin_goods['hour'] = $goods_info[6];
	        $pin_goods['begin_time'] = strtotime($begin_time);
	        $pin_goods['end_time'] = strtotime($end_time);
			
	        $pin_goods['addtime'] = time();
	        M('bargain_goods')->add($pin_goods);
	        
	        $price_arr = array('pin_price' =>$goods_info[1],'pin_count' => $goods_info[2]);
	        S($this->customer_id.'goods_price_cache'.$goods_info[0], $price_arr);
	        
	        M('goods')->where( array('goods_id' => $pin_goods['goods_id']) )->save( array('type' => 'bargain') );
