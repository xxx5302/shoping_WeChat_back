...
class ReportsController extends CommonController{
    ...
    public function index()
	{
		...
		if (empty($_GPC['reports_index']) || $_GPC['reports_index']=='0'){
			...
			//有订单的所有日期
			//$day_info2 = M()->query("select from_unixtime( pay_time, '%Y-%m-%d' ) as date from ".C('DB_PREFIX')."lionfish_comshop_order where pay_time > ".$thisweek['egt']." and pay_time < ".$thisweek['lt']." ".$condition." group by date order by date asc");
			for($i=0;$i<=$num;$i++){
				$day_info2[$i] = array(
			...
			foreach($day_info2 as $key =>$day) {
				...
				$day_info3 = M()->query("select count( * ) as count,sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(score_for_money) as score_for_money  from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$day['egt']." and date_added < ".$day['lt']." ".$condition." and (order_status_id in (7,8,9,10,12,13))");
					...
					//每天取消单数 
					$day_info4 = M()->query("select count( * ) as count,sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(score_for_money) as score_for_money , sum(fare_shipping_free) as fare_shipping_free, sum(packing_fare) as packing_fare from ".C('DB_PREFIX')."lionfish_comshop_order
		...
		if($_GPC['reports_index'] == 1){
			...
			foreach($day_info2 as $key =>$day) {
					...
					//每天取消单数 
					$day_info4 = M()->query("select count( * ) as count,sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(score_for_money) as score_for_money , sum(fare_shipping_free) as fare_shipping_free, sum(packing_fare) as packing_fare from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$day['egt']." and date_added < ".$day['lt']." ".$condition." and (order_status_id  = 5)" );
		...
		if($_GPC['reports_index'] == 2){
			...
			//有订单的所有日期
			//$day_info2 = M()->query("select from_unixtime( date_added, '%Y-%m-%d' ) as date from ".C('DB_PREFIX')."lionfish_comshop_order where pay_time > ".$thismonth['egt']." and pay_time < ".$thismonth['lt']." ".$condition." group by date order by date asc");
		...
		if($month==1 || $month==3 || $month==5|| $month==7 ||$month==8 || $month==10 ||$month==12 ){ 
			...
			//有订单的所有日期
			$day_info2 = M()->query("select from_unixtime( date_added, '%Y-%m-%d' ) as date from ".C('DB_PREFIX')."lionfish_comshop_order where pay_time > ".$lastmonth['egt']." and pay_time < ".$lastmonth['lt']." ".$condition." group by date order by date asc");
			...
			foreach($day_info2 as $key =>$day) {
				...
				//每天退款单数 
				$day_info3 = M()->query("select count( * ) as count,sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(score_for_money) as score_for_money, sum(fare_shipping_free) as fare_shipping_free from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$day['egt']." and date_added < ".$day['lt']." ".$condition." and (order_status_id in (7,8,9,10,12,13))");
	...
	public function datastatics()
	{
		...
		//今天所有订单
		$day_info = M()->query("select count( * ) as count from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$today['egt']." and date_added < ".$today['lt']." and type != 'integral' and ".$condition );
		
		$day_info2 = M()->query("select total as total,member_id as member_id,shipping_fare as shipping_fare,voucher_credit as voucher_credit,fullreduction_money as fullreduction_money,packing_fare from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$today['egt']." and date_added < ".$today['lt']." and type != 'integral' and ".$condition);
		...
		foreach($day_info2 as $key =>$val1) {
			...
			//下单金额（元）sum_money	
			$sum_money += $val1['total']+$val1['shipping_fare']+$val1['packing_fare']-$val1['voucher_credit']-$val1['fullreduction_money'];
		...
		for($i = 0;$i <= $h; $i++){
			...
			//有效销售额
			$list = M()->query("select sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(packing_fare) as packing_fare from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$todaytime['egt']." and date_added < ".$todaytime['lt']." and type != 'integral' and ".$condition." and (order_status_id in (1,2,3,4,6,11,14))" );
				
			if(empty($list[0]['total'])){
				$val = 0;
			}else{
				$val = $list[0]['total']+$list[0]['shipping_fare']+$list[0]['packing_fare']-$list[0]['voucher_credit']-$list[0]['fullreduction_money'];
		...
		for($i = 0;$i <= 23; $i++){
			...
			//有效销售额
			$list1 = M()->query("select sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(packing_fare) as packing_fare from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$yestertime['egt']." and date_added < ".$yestertime['lt']." and ".$condition."  and type != 'integral' and (order_status_id in (1,2,3,4,6,11,14))" );
				
			if(empty($list1[0]['total'])){
				$val1 = 0;
			}else{
				$val1 = $list1[0]['total']+$list1[0]['shipping_fare']+$list1[0]['packing_fare']-$list1[0]['voucher_credit']-$list1[0]['fullreduction_money'];
		...
		$sevenday_sale = M()->query("select head_id as head_id from ".C('DB_PREFIX')."lionfish_comshop_order where date_added > ".$sevenday['egt']." and date_added < ".$sevenday['lt']." and type != 'integral' and ".$condition);
		//所有团长id	
		$sale = array();		
		...
		foreach($sale as $key =>$v) {	
				...
				//团长 
				//订单数量 
				$sale2 = M()->query("select count( * ) as count  from ".C('DB_PREFIX')."lionfish_comshop_order where(head_id = ".$v['head_id'].") and date_added > ".$sevenday['egt']." and date_added < ".$sevenday['lt']." and type != 'integral' and ".$condition);
				//var_dump($sale2);
				//有效订单金额（元）
					
				$sale3 = M()->query("select sum(total) as total,sum(shipping_fare) as shipping_fare,sum(voucher_credit) as voucher_credit,sum(fullreduction_money) as fullreduction_money,sum(packing_fare) as packing_fare from ".C('DB_PREFIX')."lionfish_comshop_order where (head_id = ".$v['head_id'].") and date_added > ".$sevenday['egt']." and date_added < ".$sevenday['lt']." and type != 'integral' and ".$condition );
			
						
					//有效订单金额（元）sum_money	
				$sale_money = $sale3[0]['total']+$sale3[0]['shipping_fare']+$sale3[0]['packing_fare']-$sale3[0]['voucher_credit']-$sale3[0]['fullreduction_money'];
		...
		foreach($info as $key =>$v) {
				$g_sql = "select sum(og.quantity) as quantity ,og.name as name from ".C('DB_PREFIX')."lionfish_comshop_order_goods og,".C('DB_PREFIX')."lionfish_comshop_order o where( og.goods_id = ".$v['goods_id'].") "
	...
	private function head_order_analys($keyword,$searchtime , $starttime , $endtime, $pindex , $psize)
	{
		...
		//进行分页操作
		/*$sql = "select head_id from ".C('DB_PREFIX')."lionfish_comshop_order co left join " . C('DB_PREFIX'). "lionfish_community_head as ch on co.head_id=ch.id left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id where 1 {$where} and ch.member_id > 0 group by head_id ";
		if( !empty($keyword) ){
			$sql = "select head_id from ".C('DB_PREFIX')."lionfish_comshop_order co left join " . C('DB_PREFIX'). "lionfish_community_head as ch on co.head_id=ch.id left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id"
					." WHERE  (m.username like '%{$keyword}%' or ch.head_name like '%{$keyword}%' or ch.community_name like '%{$keyword}%')  {$where} group by head_id ";
		}
		$sql2 = "";
		if( !empty($keyword) ){
			$sql = " SELECT DISTINCT(ch.id) as head_id  FROM " . C('DB_PREFIX'). "lionfish_community_head as ch left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id
						WHERE   (m.username like '%{$keyword}%' or ch.head_name like '%{$keyword}%' or ch.community_name like '%{$keyword}%'  )  ";
		}
		if(!empty($sql2)){
		...
		$sql = " SELECT ch.id as head_id,ch.level_id  FROM " . C('DB_PREFIX'). "lionfish_community_head as ch left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id where 1=1 ";
		if( !empty($keyword) ){
		...
		if( empty($head_ids_arr) )
		{
			...
			foreach($head_ids_arr as $head_id)
			{
				...
				//有效订单量
				$effect_order_count = M('lionfish_comshop_order')->where( " head_id = {$head_id} and order_status_id in (1,4,6,11,12,14)  {$where}" )->count();
				...
				$all_order_paymoney = M('lionfish_comshop_order')->where("head_id = {$head_id} {$where}")->sum("total+shipping_fare-voucher_credit-fullreduction_money+packing_fare");
				$all_order_paymoney = sprintf('%.2f',$all_order_paymoney);
				$tmp['all_order_paymoney'] = $all_order_paymoney;
				
				//有效订单金额（元）
				$effect_order_paymoney = 	M('lionfish_comshop_order')->where("head_id = {$head_id} and order_status_id in(1,4,6,11,12,14) {$where}")->sum("total+shipping_fare-voucher_credit-fullreduction_money+packing_fare");
				...
				//退款总金额（元）
				$refund_order_paymoney = M('lionfish_comshop_order')->where(" head_id = {$head_id} and order_status_id =7 {$where} ")->sum("total+shipping_fare-voucher_credit-fullreduction_money+packing_fare");
	...
	private function head_commiss_analys( $keyword,$searchtime , $starttime , $endtime , $pindex , $psize )
	{
		...
		//进行分页操作
		/*$sql = "select head_id from ".C('DB_PREFIX')."lionfish_comshop_order co left join " . C('DB_PREFIX'). "lionfish_community_head as ch on co.head_id=ch.id left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id where {$where} and ch.member_id > 0 group by head_id ";
		if( !empty($keyword) ){
			$sql = "select head_id from ".C('DB_PREFIX')."lionfish_comshop_order co left join " . C('DB_PREFIX'). "lionfish_community_head as ch on co.head_id=ch.id left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id"
					." WHERE  (m.username like '%{$keyword}%' or ch.head_name like '%{$keyword}%' or ch.community_name like '%{$keyword}%')  {$where} group by head_id ";
		}
	...
	private function head_sale_analys( $keyword,$searchtime , $starttime , $endtime , $pindex , $psize)
	{
		...
		$sql2 = "";
		if( !empty($keyword) ){
			$sql2 = " SELECT DISTINCT (ch.id) as head_id  FROM " . C('DB_PREFIX'). "lionfish_community_head as ch left join ".C('DB_PREFIX')."lionfish_comshop_member as m on  ch.member_id = m.member_id
					WHERE  (m.username like '%{$keyword}%' or ch.head_name like '%{$keyword}%' or ch.community_name like '%{$keyword}%'  ) ";
		}
		if(!empty($sql2)){
			$sql = $sql . " union all ".$sql2;
		...
		if( empty($head_ids_arr) )
		{
			...
			foreach($head_ids_arr as $head_id)
			{
				...
				$buy_order_count = M('lionfish_comshop_order')->where(" head_id = {$head_id} and order_status_id in(1,4,6,7,11,12,14) {$where} ")->count();
				
				
				$tmp['buy_order_count'] = $buy_order_count;
				//销售额(支付的+退款的)
				
				$sum_order_paymoney = M('lionfish_comshop_order')->where("head_id = {$head_id} and order_status_id in(1,4,6,7,11,12,14) {$where}")->sum('total+shipping_fare-voucher_credit-fullreduction_money+packing_fare');
	...
	public function goodstatics(){
		...
		$sql = " SELECT og.goods_id,og.name,og.rela_goodsoption_valueid,og.price,og.cost_price,SUM( og.quantity ) as quantity, "
				...
				. " FROM ". C('DB_PREFIX'). "lionfish_comshop_order_goods as og "
				. " left join ". C('DB_PREFIX'). "lionfish_comshop_order as o on og.order_id=o.order_id "
				. " left join ". C('DB_PREFIX'). "lionfish_comshop_goods as g on g.id=og.goods_id "
