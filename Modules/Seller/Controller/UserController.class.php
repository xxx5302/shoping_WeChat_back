...
class UserController extends CommonController{
	...
	public function index()
	{
		...
		$card_list = M('lionfish_comshop_member_card')->order('id asc')->select();
		$keys_card = array();
		if( !empty($card_list) )
		{
		...
		foreach( $list as $key => $val )
		{
			...
			$ordermoney = M('lionfish_comshop_order')->where( ' type != "integral" and order_status_id in(1,2,4,6,11,14,12,13) and member_id='.$val['member_id'] )->sum('total');	
    ...
    public function agent_shenhe()
    {
        ...
        M('lionfish_comshop_member')->where( "member_id in ({$id})" )->save( array('is_apply_state' => $is_apply_state) );
    ...
    public function agent_unshenhe()
    {
        ...
        M('lionfish_comshop_member')->where( "member_id in ({$id})"  )->save( array('is_apply_state' => 2) );
	...
	public function detail()
	{
		...
		$ordermoney = M('lionfish_comshop_order')->where( ' type != "integral" and order_status_id in(1,2,4,6,11,14,12,13) and member_id='.$id )->sum('total');	
		...
		//
			if( $data['isblack'] == 1)
			{
				M('lionfish_comshop_weprogram_token')->where( array('member_id' => $id ) )->delete();
			}
		...
		if(!empty($member['card_id']) && $member['card_id'] > 0){
			$member_card = M('lionfish_comshop_member_card')->where( array('id' => $member['card_id']) )->find();
	...
	public function edit_expiretime(){
		...
		if($is_post){
			...
			M('lionfish_comshop_member')->where( array('member_id' => $id) )->save(array('card_end_time'=>$expire_time));
	...
	public function query_card(){
		...
		$page = max(1, $page);
		$page_size = 10;
		$list = M()->query('SELECT * FROM ' . C('DB_PREFIX'). 'lionfish_comshop_member_card  WHERE ' . $condition .
				' order by id desc' .' limit ' . (($page - 1) * $page_size) . ',' . $page_size );
		$total_arr = M()->query('SELECT count(1) as count FROM ' . C('DB_PREFIX') .
				'lionfish_comshop_member_card WHERE ' . $condition );
		$total = $total_arr[0]['count'];
		$s_html = "<tr>"
				. "		<th>会员卡名称</th>"
				. "		<th>原价</th>"
	...
	public function open_ucard(){
		...
		$member_card = M('lionfish_comshop_member_card')->where( array('id' => $card_id) )->find();
		$begin_time = time();
		...
		M('lionfish_comshop_member')->where( array('member_id' => $id ) )->save( $mb_up_data );
	...
	public function user_shenhe()
	{
		...
		if( $state == 1 )
		{			
			M('lionfish_comshop_member')->where( array('member_id' => $member_id) )->save( array('is_comsiss_audit' => 1) );
			
		}else{
			
			M('lionfish_comshop_member')->where( array('member_id' => $member_id) )->save( array('is_comsiss_audit' => 0) );
		}
