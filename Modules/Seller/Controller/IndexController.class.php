...
class IndexController extends CommonController {
	...
	public function analys ()
	{
		...
		$score_for_money_info = M('lionfish_comshop_order')->field(' sum(score_for_money) as score_for_money')->where("1 ".$score_for_money_where )->find();
		...
		$sum_score = M('lionfish_comshop_member_integral_flow')->where(' addtime > '.$today_time.' and type = "orderbuy" and in_out = "out"  ')->sum('score');
		if(empty($sum_score)){
			$sum_score = 0;
		}
	...
	public function order_count()
	{
		...
		{
			$order_goods = M('lionfish_comshop_order_goods')->field('supply_id')->where( array('order_id' => $val ) )->find();
		...
		if (!defined('ROLE') || ROLE != 'agenter' ){
			...
			if(!empty($is_open_supply_voice)){
				//获取供应商id   
				$supply_id = $_SESSION["oscshop_seller_s"]["agent_auth"]["uid"];
	...
	{
		//third_delivery_query($order)
		$order_model = D('Home/Order');

		$order = M('lionfish_comshop_order')->where( " order_id = 7229 ")->find();
			

        $array = $order_model->third_delivery_query($order);
