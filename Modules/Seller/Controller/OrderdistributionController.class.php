...
class OrderdistributionController extends CommonController{
	...
	public function index()
	{
        ...
        $list = M()->query('SELECT a.*,b.storename FROM ' . C('DB_PREFIX'). "lionfish_comshop_orderdistribution a"
              . " left join ".C('DB_PREFIX')."lionfish_comshop_supply b on a.store_id=b.id "
              . " WHERE 1=1 "
    ...
    public function distribution_list()
    {
        ...
        $list = M()->query("SELECT a.*,o.order_num_alias,o.date_added FROM " . C('DB_PREFIX'). "lionfish_comshop_orderdistribution_order a  "
                         . " left join " . C('DB_PREFIX'). "lionfish_comshop_order o on a.order_id=o.order_id "
        ...
        if( !empty($list) )
        {
            ...
            {
                //配送员名称
                $distribution_info = M('lionfish_comshop_orderdistribution')->field('username')->where( array('id' => $val['orderdistribution_id'] ) )->find();
                $val['username'] = $distribution_info['username'];
                //下单时间
                $val['date_added'] = date('Y-m-d H:i:s',$val['date_added']);
                //抢单时间
                $order_log = M('lionfish_comshop_orderdistribution_log')->field('addtime')->where( array('order_id' => $val['order_id'],'state'=> 2) )->find();
                if(!empty($order_log['addtime'])) {
                    $val['rob_time'] = date('Y-m-d H:i:s', $order_log['addtime']);
                }
                //取货时间
                $order_log = M('lionfish_comshop_orderdistribution_log')->field('addtime')->where( array('order_id' => $val['order_id'],'state'=> 3) )->find();
                if(!empty($order_log['addtime'])) {
                    $val['receive_time'] = date('Y-m-d H:i:s', $order_log['addtime']);
                }
                //完成时间
                $order_log = M('lionfish_comshop_orderdistribution_log')->field('addtime')->where( array('order_id' => $val['order_id'],'state'=> 4) )->find();
                if(!empty($order_log['addtime'])){
        ...
        $count_list = M()->query("SELECT count(1) as count FROM " . C('DB_PREFIX'). "lionfish_comshop_orderdistribution_order a  "
                . " left join " . C('DB_PREFIX'). "lionfish_comshop_order o on a.order_id=o.order_id "
                . " WHERE 1=1 " . $condition);
