...
class SalesroomMemberModel{
	...
	public function update($data)
	{
		...
		{
			unset($ins_data['id']);
			M('lionfish_comshop_salesroom_member')->where( array('id' => $id) )->save( $ins_data );
		}else{
			$ins_data['addtime'] = time();
			$id = M('lionfish_comshop_salesroom_member')->add( $ins_data );
		}
		if(!empty($data['salesroom_ids'])){
			M('lionfish_comshop_salesroom_relative_member')->where( array('smember_id' => $id) )->delete();
			...
			foreach($salesroom_ids as $v){
				...
				M('lionfish_comshop_salesroom_relative_member')->add( $salesroom_relative_member );
			}
		}
	}
