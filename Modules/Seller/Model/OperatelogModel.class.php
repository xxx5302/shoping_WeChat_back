...
class OperatelogModel{
	...
	/*
	 DROP TABLE IF EXISTS `oscshop_lionfish_comshop_systemoperation_log`;
	CREATE TABLE `oscshop_lionfish_comshop_systemoperation_log` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='狮子鱼商城系统操作日志表' AUTO_INCREMENT=1 ;
	...
	*/
	...
	public function addOperateLog($operation_type,$content)
	{
		...
		if (defined('ROLE') && ROLE == 'agenter' ) {
			$operation_seller_id = $_SESSION["oscshop_seller_s"]["agent_auth"]["uid"];
			$items = M('lionfish_comshop_supply')->field('name')->where( array('id' =>$operation_seller_id ) )->find();
			$operation_seller_name ='供应商--'.$items['name'];
		}else{
		...
		M('lionfish_comshop_systemoperation_log')->add($ins_data);
