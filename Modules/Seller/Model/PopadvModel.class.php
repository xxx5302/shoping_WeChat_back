...
class PopadvModel{
	...
	public function update($_GPC)
	{
		...
		{
			$add_time = time();
			M('lionfish_comshop_pop_adv')->where( array('id' => $id) )->save( $ins_data );
			$aid = $_GPC['aid'];
			$thumb = $_GPC['thumb'];
			$adv_id = $_GPC['adv_id'];
			M('lionfish_comshop_pop_adv_list')->where( array('ad_id' => $id) )->delete();
			for($i = 0;$i < count($aid);$i++){
				...
				M('lionfish_comshop_pop_adv_list')->add($adv_data);
			}
		}else{
		    $add_time = time();
			$ins_data['addtime'] = $add_time;
			$adv_id = M('lionfish_comshop_pop_adv')->add($ins_data);
