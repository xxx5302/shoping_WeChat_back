...
class CommonorderModel{
	...
	**/
	public function get_order_goods_paymoney( $order_goods_id )
	{
		...
		if( empty($order_goods_info) )
		{
			...
			$order_info = M('lionfish_comshop_order')->field('type')->where( array('order_id' => $order_goods_info['order_id']) )->find();
	...
	**/
	...
	{
	    $total_money = M('lionfish_comshop_order_goods_refund')->where( array('order_id' =>$order_id ) )->sum('money');
	...
	{
	    $integral_flow_info = M('lionfish_comshop_member_integral_flow')->where( array('order_id' => $order_id, 'order_goods_id' =>$order_goods_id,'type' => 'orderbuy' ) )->find();
	...
	public function refund_order_goods_intrgral( $order_id, $order_goods_id ,$score )
	{
	    ...
	    $res = M('lionfish_comshop_order_goods_refund_intrgral')->add($in_data);
	...
	**/
	...
	{
		$order_goods_list = M('lionfish_comshop_order_goods')->field('order_goods_id,quantity,shipping_fare')->where( array('order_id' => $order_id , 'is_refund_state' => 0) )->select();
		if( !empty($order_goods_list) )
		{
			foreach( $order_goods_list as $val)
			{
				...
				if( $is_back_buyscore == 0)
				{
				    ...
				    if( !empty($integral_flow_info) )
				    {
				        $has_refflow_score = M('lionfish_comshop_member_integral_flow')->where( array('order_id' => $order_id, 'order_goods_id' =>$order_goods_id,'type' => 'refundorder' ) )->sum('score');
				...
				{
                    $has_refund_shipping_fare = M('lionfish_comshop_order_goods_refund')->where( array('order_goods_id' => $val['order_goods_id'] ) )->sum('refund_shipping_fare');
