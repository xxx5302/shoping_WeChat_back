...
class VipcardController extends CommonController {
	...
	public function get_vipcard_baseinfo()
	{
		...
		$member_info['username'] = str_replace('&#039;',"'",$member_info['username']);
	...
	**/
	public function wxcharge()
	{
		...
		if($card_info['price'] == 0){//会员卡价格为0时
			D('Home/Vipcard')->member_charge_zero_card($order_id);
			$data['code'] = 0;
			$data['is_zero_pay'] = 1;//1、0元支付成功标识；0、微信支付
		}else{
