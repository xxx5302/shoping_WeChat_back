...
class SupplyController extends CommonController {
	...
	public function get_apply_page()
	{
	    ...
	    if(!empty($info['value'])){
	    	echo json_encode( array('code' => 0, 'data' => htmlspecialchars_decode(htmlspecialchars_decode($info['value'])) , 'supply_diy_name' => $supply_diy_name ) );
			die();
	    }else{
	...
	public function get_list()
	{
		...
		foreach( $list as $key => &$val )
		{
			...
			// 统计商品数量
			$goods_supply_id_arr = $goods_supply_nolimit_arr = array();
			$goods_supply_id_arr = M()->query("SELECT g.id FROM ".C('DB_PREFIX')."lionfish_comshop_good_common  as gc , ".C('DB_PREFIX')."lionfish_comshop_goods as g, 
								".C('DB_PREFIX')."lionfish_community_head_goods as hg WHERE gc.goods_id = g.id and gc.goods_id = hg.goods_id and gc.supply_id = {$val['id']} 
								and g.grounding = 1 and gc.begin_time < {$now_time}  and gc.end_time > {$now_time} and hg.head_id = {$head_id} ");
								
								
			$goods_supply_nolimit_arr = M()->query("SELECT g.id FROM ".C('DB_PREFIX')."lionfish_comshop_goods as g, ".C('DB_PREFIX')."lionfish_comshop_good_common as gc WHERE gc.goods_id = g.id and gc.supply_id = {$val['id']}  and g.is_all_sale=1 and g.grounding = 1 and gc.begin_time < {$now_time}  and gc.end_time > {$now_time}  ");
			
			$goods_all = $goods_all_arr = array();
			$goods_all = array_merge($goods_supply_id_arr, $goods_supply_nolimit_arr);
			foreach($goods_all as $vv){
				$goods_all_arr[] = $vv['id'];
			}
			$goods_all_arr = array_unique($goods_all_arr);
			$val['goods_count'] = count($goods_all_arr);

			$val['goods_supply_id_arr'] = $goods_supply_id_arr;
			$val['goods_supply_nolimit_arr'] = $goods_supply_nolimit_arr;
	...
	public function get_details()
	{
		...
		$is_only_distribution = max(0, intval($_GPC['is_only_distribution']));
		$psize = 20;
		$head_id = intval($_GPC['head_id']);
		$token = $_GPC['token'];
		...
		if(!empty($item)) {
			...
			if($is_only_distribution) {
				$where .= " and gc.is_only_distribution = " . $is_only_distribution;
			}
			$community_goods = D('Home/Pingoods')->get_community_index_goods('g.*,gc.begin_time,gc.end_time,gc.big_img,gc.is_take_fullreduction,gc.labelname,gc.goods_start_count,gc.oneday_limit_count, gc.total_limit_count, gc.one_limit_count ', $where, $offset, $per_page);
