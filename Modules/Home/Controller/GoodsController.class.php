...
class GoodsController extends CommonController {
	...
	**/
	...
	public function get_seller_quan()
	{
		...
		$quan_list2 = M()->query("select * from ".C('DB_PREFIX')."lionfish_comshop_coupon where 1 {$where2} order by displayorder desc ,id asc limit 100 ");
	...
	**/
	...
	public function get_goods_detail() {
					...
					if(  $community_info['state'] == 1 && $community_info['enable'] == 1 )
					{
						...
						if($is_head_takegoods == 1)
						{
							//开启 团长等级
								...
								//是否设置了团长等级
								$head_level = M('lionfish_comshop_community_head_level')->find();
								$is_modify_head_commission = M('lionfish_comshop_good_common')->field('is_modify_head_commission,is_community_head_commission,community_head_commission')->where( array('goods_id' => $goods['goods_id'] ) )->find();
								...
								if(!empty($head_level) && $is_modify_head_commission["is_modify_head_commission"] == 1 ){
									...
									if($level == 0 ){
										$head_commission_info['community_head_commission'] = D('Home/Front')->get_config_by_name('default_comunity_money');
											...
											$community_head_level = M('lionfish_comshop_community_head_level')->where(array('id' => $level ) )->find();	
								...
								$is_community_head_commission = M('lionfish_comshop_good_common')->field('is_community_head_commission,community_head_commission')->where( array('goods_id' => $goods['goods_id'] ) )->find();
		...
		**/
		...
		$goods_supply_info = M('lionfish_comshop_good_common')->where( array('goods_id' => $goods['id'] ) )->find();
    ...
    public function get_instructions()
    {
		...
		if( $goods_id > 0 )
		{
			...
			$gd_info = M('lionfish_comshop_good_common')->field('supply_id')->where( array('goods_id' => $goods_id )  )->find();	
						
			if( !empty($gd_info) && $gd_info['supply_id'] > 0 )
			{
				$su_info = M('lionfish_comshop_supply')->field('qualifications')->where( array('id' => $gd_info['supply_id'] ) )->find();	
	...
	public function goods_guess_like()
	{
		...
		if(!empty($community_id)){
			...
			//团长商品id
			 ...
			 {
			     $sql_likegoods = "select g.*,gc.end_time,gc.begin_time,gc.oneday_limit_count,gc.total_limit_count,gc.one_limit_count,gc.goods_start_count from ".C('DB_PREFIX')."lionfish_comshop_goods as g,".C('DB_PREFIX')."lionfish_comshop_good_common as gc
