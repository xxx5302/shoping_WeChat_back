...
class LocaltownController extends CommonController {
    ...
    public function send_data()
    {
        ...
        $list = M('lionfish_comshop_orderdistribution_order')->where( array('state' => 1) )->select();
	...
	**/
    public function get_localtown_delivery()
    {
        ...
        if(!empty($orderdistribution_info['store_id'])){//独立供应商订单
	...
	**/
    ...
    public function get_distribution_commission_info()
    {
        ...
        $orderdistribution_info = M('lionfish_comshop_orderdistribution')->where( array('member_id' => $member_id) )->find();
        ...
        $member_commiss = M('lionfish_comshop_orderdistribution_commiss')->where( array('orderdistribution_id' => $orderdistribution_info['id'] ) )->find();
        ...
        $last_weixin_info = M('lionfish_comshop_orderdistribution_tixian_order')->where( array('member_id' => $member_id, 'type' => 2 ) )->find();
        ...
        $last_alipay_info = M('lionfish_comshop_orderdistribution_tixian_order')->where( array('member_id' => $member_id, 'type' => 3 ) )->find();
        ...
        $last_bank_info = M('lionfish_comshop_orderdistribution_tixian_order')->where( array('member_id' => $member_id, 'type' => 4 ) )->find();
    ...
    public function tixian_sub()
    {
        ...
        $member_commiss = M('lionfish_comshop_orderdistribution_commiss')->where( array('member_id' => $member_id ) )->find();
...

        {
            ...
            M('lionfish_comshop_orderdistribution_tixian_order')->add($data);
            ...
            M('lionfish_comshop_orderdistribution_commiss')->where( array('member_id' => $member_id ) )->setInc('money',-$money);
            M('lionfish_comshop_orderdistribution_commiss')->where( array('member_id' => $member_id ) )->setInc('dongmoney',$money);
    ...
    public function tixian_record()
    {
        ...
        $list = M('lionfish_comshop_orderdistribution_tixian_order')->where( array('member_id' =>$member_id ) )->order('addtime desc')->limit($offset,$per_page )->select();
	...
	**/
	public function rob_distribution_order()
	{
        ...
        if(!empty($localtown_grabbing_distance) && is_numeric($localtown_grabbing_distance)){
            $distribution_order =  M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_id ) )->find();
            // 计算配送员距离
	...
	**/
	public function distribution_arrived_order(  )
	{
		...
		$order_supplyid = M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id) )->find();
    ...
    public function change_distribution_notice(){
        ...
        $res = M('lionfish_comshop_orderdistribution')->where( array('member_id' => $member_id) )->save(array('is_new_notice'=>$is_new_notice));
        if($res !== false){
