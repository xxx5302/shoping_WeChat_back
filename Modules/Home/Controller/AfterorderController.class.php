...
class AfterorderController extends CommonController {
	...
	public function get_order_money()
	{
		...
		{
			$ref_shipping_fare_old = M('lionfish_comshop_order_refund')->where( array('order_goods_id' => $order_goods_id,'state' => 3 ) )->sum('ref_shipping_fare');
		...
		$refund_integral_flow = M('lionfish_comshop_member_integral_flow')->where( array('type' => 'refundorder', 'order_goods_id' => $order_goods_id ) )->sum('score');
	...
	public function refund_sub()
	{
		...
		if( $real_refund_quantity + $order_goods['has_refund_quantity'] == $order_goods['quantity'] ){
			...
			$ref_shipping_fare_old = M('lionfish_comshop_order_goods_refund')->where( array('order_goods_id' => $order_goods_id ) )->sum('refund_shipping_fare');
			...
			**/
			...
			if( !empty($weixin_template_apply_refund) && !empty($platform_send_info_member_id) )
			{
				...
				if( !empty($weixin_appid) && !empty($weixin_template_apply_refund) )
				{
                    ...
                    //dump($weopenid);die;
					$sql = "select * from oscshop_lionfish_comshop_member where member_id in ( ".$platform_send_info_member_id.")";
