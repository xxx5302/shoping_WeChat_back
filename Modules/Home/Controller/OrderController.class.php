...
class OrderController extends CommonController {
	...
	**/
	public function del_cancle_order()
	{
		...
		$order_member_name = M('lionfish_comshop_member')->where( array('member_id' => $member_id ) )->find();
	...
	public function order_info()
	{
		...
		if( $is_supply > 0 )
		{
			...
			{
				$ck_order_goods = M('lionfish_comshop_order_goods')->where( array('supply_id' => $supply_info['id'], 'order_id' =>  $order_id ) )->find();
				
				if( !empty($ck_order_goods) )
				{
					$order_info = M('lionfish_comshop_order')->where( array('order_id' => $ck_order_goods['order_id']) )->find();
				}else{
		...
		{
			$order_info['orderdistribution_order'] = M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_info['order_id'] ) )->find();
			if( !empty($order_info['orderdistribution_order']) && $order_info['orderdistribution_order'] > 0 )
			{
				$orderdistribution = M('lionfish_comshop_orderdistribution')->where( array('id' => $order_info['orderdistribution_order']['orderdistribution_id']  ) )->find();
		...
		if( $order_info['delivery'] == 'hexiao' ){
			if($order_info['store_id'] == 0){
				...
				$store_info = M('lionfish_comshop_supply')->where( array('id' => $order_info['store_id'] ) )->find();
	    ...
	    foreach($order_goods_list as $key => $order_goods)
	    {
		   ...
		   $order_goods['hascomment'] = 0;
			...
			//核销订单
			if( $order_info['delivery'] == 'hexiao' ){
				$order_goods_saleshexiao = M('lionfish_comshop_order_goods_saleshexiao')->where( array('order_id' =>$order_id,'order_goods_id' => $order_goods['order_goods_id'] ) )->find();
				//有效期截止
			...
			//ims_   ims_lionfish_comshop_order_goods_refund addtime
			
			$order_goods_refund_list = M('lionfish_comshop_order_goods_refund')->field('real_refund_quantity,money,addtime')->where( "order_goods_id=".$order_goods['order_goods_id'] )->order('id asc')->select();
	...
	public function orderlist()
	{
		...
		else if( $is_supply == 1 )
		{
			...
			if( !empty($supply_info) &&  $supply_info['state'] == 1 && $supply_info['type'] == 1 && $supply_info['is_open_mobilemanage'] == 1 )
			{
				...
				if( !empty($keyword) )
				{
					$left_join = ' inner join ( select DISTINCT(og.order_id) from ' . C('DB_PREFIX') . 'lionfish_comshop_order_goods as og  where  (locate("'.$keyword.'",og.name)>0) and og.supply_id = '.$supply_info['id'].' ) gs on gs.order_id=o.order_id';
				}else{
					$left_join = ' inner join ( select DISTINCT(og.order_id) from ' . C('DB_PREFIX') . 'lionfish_comshop_order_goods as og  where   og.supply_id = '.$supply_info['id'].' ) gs on gs.order_id=o.order_id';
				}
	    ...
	    foreach($list as $key => $val)
	    {
	        ...
	        foreach($goods_list as $kk => $vv) 
	        {
				...
				if($is_tuanz == 1){
					...
					$order_goods_refund_list = M('lionfish_comshop_order_goods_refund')->where( array('order_id' => $val['order_id'],'order_goods_id' => $vv['order_goods_id'] ) )->select();
			...
			{
				$val['orderdistribution_order'] = M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $val['order_id'] ) )->find();
				if( !empty($val['orderdistribution_order']) && $val['orderdistribution_order'] > 0 )
				{
					$orderdistribution = M('lionfish_comshop_orderdistribution')->where( array('id' => $val['orderdistribution_order']['orderdistribution_id']  ) )->find();
	...
	function receive_order()
	{
		...
		if($order_info['delivery'] == 'localtown_delivery'){
			...
			M('lionfish_comshop_orderdistribution_order')->where( array('order_id' => $order_id) )->save( array('state' => 4) );
	...
	**/
	...
	public function order_head_info()
	{
	    ...
	    foreach($order_goods_list as $key => $order_goods)
	    {
	       ...
	       //
			...
			$order_goods_refund_list = M('lionfish_comshop_order_goods_refund')->where( "order_id={$order_id} and order_goods_id=".$order_goods['order_goods_id'] )->select();
	...
	public function order_share_info(){
	    ...
	    foreach($order_goods_list as $key => $order_goods)
	    {
			...
			$order_option_info = M('lionfish_comshop_order_option')->field('value')->where( array('order_id' => $order_id ,'order_goods_id' => $order_goods['order_goods_id'] ) )->select();
			...
			**/
			...
			$order_goods_comment_info =	M('lionfish_comshop_order_comment')->field('comment_id')->where( array('order_id' => $order_id, 'goods_id' => $order_goods['goods_id'] ) )->find();
			...
			$order_goods_refund_list = M('lionfish_comshop_order_goods_refund')->field('real_refund_quantity,money,addtime')->where( array('order_goods_id' =>$order_goods['order_goods_id'] ) )->order('id asc')->select();
	...
	public function order_pay(){
		...
		if($is_supply == 1){
			...
			if(empty($supply_info)){
				$result['code'] = 1;
				$result['msg'] = '您不是供应商，无法操作';
				echo json_encode($result);
				die();
			}
			$order_info = M('lionfish_comshop_order')->where( array('store_id' => $supply_info['id'],'order_id' => $order_id) )->find();
		}
		if(empty($order_info)){

			$result['code'] = 1;

			$result['msg'] = '非法操作,供应商不存在该订单';
	...
	public function order_changeprice(){
		...
		foreach($order_goods_ids as $k=>$order_goods_id){
			...
			if(!empty($change_prices[$k])){
				...
				M('lionfish_comshop_order_goods')->where( array('order_id' =>$order_id,'order_goods_id' => $order_goods_id  ) )->save( $update_data);
			}
		}
		$order_info = M('lionfish_comshop_order')->where( array('order_id' =>$order_id) )->find();
		...
		M('lionfish_comshop_order')->where( array('order_id' =>$order_id) )->save($order_data);
	...
	public function order_delivery(){
		...
		D('Seller/Order')->do_send_localtown($order_id,'供应商操作，开始配送货物');
		$result['code'] = 0;
		echo json_encode($result);
		die();
	}
	...
	public function choosemember()
	{
		...
		分页结束
		 **/
		$sql = 'SELECT * FROM ' . C('DB_PREFIX') . 'lionfish_comshop_orderdistribution  where '
				. $condition .
	...
	public function sub_orderchoose_distribution(){
		...
		if($is_supply == 1){
			$supply_info = M('lionfish_comshop_supply')->where( array('member_id' => $member_id) )->find();
			if(empty($supply_info)){
				echo json_encode(array('code' => 1, 'msg' => '您不是供应商，无法操作'));
				die();
			}
			$order_info = M('lionfish_comshop_order')->where( array('store_id' => $supply_info['id'],'order_id' => $order_id) )->find();
		}
		if(empty($order_info)){
			echo json_encode(array('code' => 1, 'msg' => '非法操作,供应商不存在该订单'));
			die();
		}
		if($order_info['order_status_id'] != 4)
		{
		...
		$distribution_info = M('lionfish_comshop_orderdistribution')->where(array('id'=>$distribution_id,'store_id'=>$supply_info['id']))->find();
	...
	public function share_order(){
		...
		$order_goods_list = M('lionfish_comshop_order_goods')->where( array('order_id' => $order_id) )->order('order_goods_id asc')->select();
		...
		if($order_info['delivery'] != 'hexiao'){
			$head_id = $order_info['head_id'];
			$head_info = M('lionfish_community_head')->where(array('id'=>$head_id))->field('id,member_id,community_name,head_name,head_mobile,province_id,city_id,country_id,area_id,address')->find();
			$head_member_info = M('lionfish_comshop_member')->where(array('member_id'=>$head_info['member_id']))->find();
