...
class IndexController extends CommonController {
	...
	记录访客信息
	*/
	public function write_access_records(){
        $_GPC = I('request.');
        $records_data = array();
        $records_data['nick_name'] = $_GPC['nickName'];
        $records_data['country'] = $_GPC['country'];
        $records_data['province'] = $_GPC['province'];
        $records_data['city'] = $_GPC['city'];
        $records_data['gender'] = $_GPC['gender'];
        $records_data['model'] = $_GPC['model'];
        $records_data['access_time'] = date("Y-m-d H:i:s");
        $result = M('access_records')->add($records_data);
        echo $result;
	}
	...
	**/
	public function get_firstload_msg()
	{
		...
		if( !empty($token) )
		{
			...
			$community_history = M('lionfish_community_history')->field('head_id')->where( array('member_id' => $weprogram_token['member_id'] ) )->order('addtime desc')->find();
			
								
			if( !empty($community_history) )
			{
				$cur_community_info = M('lionfish_community_head')->field('id')->where( array('id' => $community_history['head_id'] ) )->find();
            ...
            {
                $now_member_info = M('lionfish_comshop_member')->where( array('member_id' => $weprogram_token['member_id'] ))->find();
	...
	public function get_advimg(){
		$data = M('lionfish_comshop_advimg')->order('id desc')->find();
		if(!empty($data)) {
