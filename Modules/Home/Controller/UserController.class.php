...
class UserController extends CommonController {
	...
	public function myvoucherlist()
	{
		...
		$category_list = M('lionfish_comshop_coupon_category')->field('id,name,status')->where( array('merchid' => 0) )->order('id desc')->select();
...
//		M('goods')->field('image')->where( array('goods_id' => ) )->find();
...

	{
		...
		$comment_id = M('lionfish_comshop_order_comment')->add($data);
		
		if(isset($data['state']) && $data['state'] == 1){
		    //好评有礼送积分
		    D('Seller/Order')->sendCommentGift($comment_id);
		    M('lionfish_comshop_order_comment')->where(array('comment_id'=>$comment_id,'member_id'=>$member_id))->save(array('is_send_point'=>1));
		}
	  //TOD...暂时屏蔽
	...
	**/
	public function get_user_info()//
	{
		...
		if($member_id){
			...
			//判断是否是核销员
			$salesroom_member_info = M('lionfish_comshop_salesroom_member')->where( array('member_id' => $member_id,'state'=>1) )->find();
	...
	public function applogin_do()
	{
        ...
        {
            $now_member_info = M('lionfish_comshop_member')->where( array('member_id' => $member_id))->find();
	...
	**/
	public function get_copyright()
	{
		...
		$commiss_diy_name = D('Home/Front')->get_config_by_name('commiss_diy_name');
		//新增我的--帮助文章显示内容，通过更改数据表lionfish_comshop_config----commiss_add_name字段值---by李伟男
        $commiss_add_name = D('Home/Front')->get_config_by_name('commiss_add_name');
		...
		echo json_encode(
			array(
				...
				'commiss_add_name' => $commiss_add_name,
	...
	public function get_user_distribution_order()
	{
		...
		$offset = ($page - 1) * $per_page;
		$list = array();
		$sql = "select id,order_id,state,shipping_money,is_statement,addtime from  ".C('DB_PREFIX')."lionfish_comshop_orderdistribution_order "
		...
		if( !empty($list) )
		{
			...
			{
				$value['addtime'] = date('Y-m-d H:i:s', $value['addtime']);
				$order_info = M('lionfish_comshop_order')->field('order_num_alias')->where( array('order_id' => $value['order_id'] ) )->find();
    ...
    public function save_formData()
    {
        ...
        M('lionfish_comshop_member')->where( array('member_id' => $member_id) )->save( $up_data );
	...
	public function collect_voucher()
	{
		...
		$list = M('lionfish_comshop_coupon')->where($params)->order('add_time desc')->select();
		...
		foreach($list as $key => $val)
		{
			...
			//验证买家是否有 优惠券
			$count = M('lionfish_comshop_coupon_list')->where(array('voucher_id'=>$val['id'],'user_id'=>$member_id))->count();
			if($count > 0){
				$val['is_collect'] = 1;
			}else{
				$val['is_collect'] = 0;
			...
			if($val['is_collect'] == 1){
				$no_use_count = M('lionfish_comshop_coupon_list')->where(array('voucher_id'=>$val['id'],'user_id'=>$member_id,'consume'=>'N'))->count();
				if($no_use_count > 0){
					$val['is_use'] = 0;
				}else{
					$val['is_use'] = 1;
			...
			// }else{
				if($val['is_collect'] == 1){
					$no_over_count = M('lionfish_comshop_coupon_list')->where(array('voucher_id'=>$val['id'],'user_id'=>$member_id))->where('end_time > '.$nowtime)->count();
					if($no_over_count > 0){
						$val['is_over'] = 0;
					}else{
						$val['is_over'] = 1;
