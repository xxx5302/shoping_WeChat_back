...
class SolitaireController extends CommonController {
	...
	**/
	public function sub_head_solitaire()
	{
		...
		$need_data['head_id_list'] = array($head_id);
	
		$res = D('Seller/Solitaire')->updatedo($need_data, $_W['uniacid'] ,$addtype , $appstate );
		...
		if( !empty( $list ) )
		{
			...
			foreach( $list as $key => $val )
			{
				...
				if($state!==0) {
					...
					//访问人头像列表
					$avatar_sql = "select A.id, B.avatar from ".C('DB_PREFIX')."lionfish_comshop_solitaire_invite AS A, ".C('DB_PREFIX')."lionfish_comshop_member AS B where  A.soli_id={$val['id']} AND A.member_id=B.member_id group by A.member_id limit 6";
					$invite_list =  M()->query($avatar_sql);
				}
				
				
				$tmp_arr['order_count'] = $order_count;
				
				$tmp_arr['invite_list'] = $invite_list;
			...
			echo json_encode( array('code' => 0,'head_data' => $head_data, 'data' => $need_data, 'showTabbar' => $showTabbar ) );
			die();
		}else{
			
			echo json_encode( array('code' => 1, 'head_data' => $head_data, 'showTabbar' => $showTabbar ) );
			die();
		}
	...
	**/
	public function get_solitaire_detail()
	{
		...
		if($is_head==1) {
			...
			{
				echo json_encode( array('code' => 1, 'msg'=>"无权限访问此接龙") );
				die();
			}
		}
		/*else {
...
//		$solitaire_goods_str = "";
		...
		if( !empty( $solitaire_goods_arr ) )
		{
			...
			{
				$tp_arr[] = $val['goods_id'];
                $gd_info_list[] =  M()->query('select g.*,gc.begin_time,gc.end_time,gc.big_img,gc.is_take_fullreduction,gc.labelname,gc.video,gc.goods_start_count from 
				'.C('DB_PREFIX')."lionfish_comshop_goods as g ,".C('DB_PREFIX')."lionfish_comshop_good_common as gc 
					where  g.id = {$val['goods_id']} and g.id =gc.goods_id  ");
			}
...
//					where  g.id in(".$solitaire_goods_str.") and g.id =gc.goods_id  ");
			...
			foreach($gd_info_list as  $gd_infos )
			{
                $gd_info = $gd_infos[0];
				$tmp_data = array();
				...
				$tmp_data['goods_start_count'] = $gd_info['goods_start_count'];
		...
		//群接龙留言功能是否开启
		$solitaire_is_message = D('Home/Front')->get_config_by_name('solitaire_is_message');
		if( empty($solitaire_is_message) )
		{
		...
		echo json_encode(  array('code' => 0, 
							...
							'solitaire_is_message' => $solitaire_is_message,
	...
	**/
	public function get_member_solitairelist()
	{
		...
		$list =  M()->query('select gc.* from '.C('DB_PREFIX')."lionfish_comshop_order as g ,".C('DB_PREFIX')."lionfish_comshop_solitaire as gc 
					where g.member_id={$member_id} and g.soli_id>0 and g.soli_id = gc.id and gc.head_id={$head_id} order by g.order_id desc limit {$offset}, {$size} " );
