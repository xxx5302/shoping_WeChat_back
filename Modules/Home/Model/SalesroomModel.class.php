...
class SalesroomModel{
	...
	public function get_goods_salesroom($goods_id,$supply_id,$field="*")
	{
	    ...
	    $salesroom_base = M('lionfish_comshop_goods_salesroombase')->where(array('goods_id'=>$goods_id))->find();
		if($salesroom_base['hx_assign_salesroom'] == 0){
			$list =  M('lionfish_comshop_salesroom')->where(array('supply_id'=>$supply_id,'state'=>1))->field($field)->select();
		}else{
			$goods_salesroom_list =   M('lionfish_comshop_goods_relative_salesroom')->where(array('goods_id'=>$goods_id))->field('salesroom_id')->select();
			if(!empty($goods_salesroom_list)){
				...
				$list =  M('lionfish_comshop_salesroom')->where(array('supply_id'=>$supply_id,'state'=>1))->where("id in (".$salesroom_ids.")")->field($field)->select();
			}
		}
	    return $list;
	}
		...
		if(!empty($saleshexiao_list)){
			foreach($saleshexiao_list as $sk=>$sv){
				...
				if($salesroom_base['hx_assign_salesroom'] == 0){
					...
					$goods_salesroom_list = M('lionfish_comshop_order_goods_relative_salesroom')->where(array('goods_id'=>$goods_id,'order_id'=>$order_id))->field('salesroom_id')->select();
					if(!empty($goods_salesroom_list)){
						foreach($goods_salesroom_list as $k=>$v){
							if(empty($salesroom_ids)){
	...
	public function get_today_hexiao_record_by_smember_id($smember_id){
		...
		if(!empty($saleshexiao_record_list)){
			foreach($saleshexiao_record_list as $k=>$v){
				...
				//商品信息
				$order_goods_info = M('lionfish_comshop_order_goods')->where(array('order_goods_id'=>$order_goods_id))->field('goods_id,name as goods_name,goods_images')->find();
	...
	public function get_hexiao_order_by_code($hexiao_volume_code,$salesmember_id){
		...
		//shipping_tel  收货号码
		$order_info = M('lionfish_comshop_order')->where(array('hexiao_volume_code'=>$hexiao_volume_code))->field($field)->find();
		$order_goods_saleshexiao_list = array();
		$order_result['is_exist'] = 1;
		if(empty($order_info)){
			
			$order_goods_saleshexiao_info = M('lionfish_comshop_order_goods_saleshexiao')->where(array('hexiao_volume_code'=>$hexiao_volume_code))->find();
			if(!empty($order_goods_saleshexiao_info)){
				$state = 1;
				//扫码
				$order_info = M('lionfish_comshop_order')->where(array('order_id'=>$order_goods_saleshexiao_info['order_id']))->field($field)->find();
				$order_goods_saleshexiao_list[0] = $order_goods_saleshexiao_info;
			}else{
				$state = 3;
				//手机号  1 4 5  6 7  11 12 14
				$order_list = M('lionfish_comshop_order')->where(array('shipping_tel'=>$hexiao_volume_code , 'delivery' =>'hexiao'))->where(' order_status_id = 4 || order_status_id = 11 ')->field('order_id')->select();
				if(empty($order_list)){
				...
				$order_goods_saleshexiao_list = M('lionfish_comshop_order_goods_saleshexiao')->where(' order_id in('.$order_id.') ')->order('is_hexiao_over asc,addtime asc')->select();
				...
				$order_info = M('lionfish_comshop_order')->where(array('order_id'=>$order_list[0]['order_id']))->field($field)->find();

			}
		}else{
			//核销码
			$state = 2;
			$order_goods_saleshexiao_list = M('lionfish_comshop_order_goods_saleshexiao')->where(array('order_id'=>$order_info['order_id']))->select();	
		}
		...
		if(!empty($order_info)){
			...
			foreach($order_goods_saleshexiao_list as $k=>$v){
				if($this->check_goods_relative_smember($v['order_id'],$v['order_goods_id'],$v['goods_id'],$salesmember_id) == 1){
					$order_num_alias = M('lionfish_comshop_order')->where(array('order_id'=>$v['order_id']))->field('order_num_alias,order_status_id')->find();
					$order_goods_info = M('lionfish_comshop_order_goods')->where(array('order_goods_id'=>$v['order_goods_id']))->field('goods_id,name as goods_name,goods_images,quantity')->find();
