...
class OrderModel extends Model{
	...
	public function addThirthQuery($thirth_data){
		...
		$thirth_query = M('lionfish_comshop_orderdistribution_thirth_query')->where(array('order_id'=>$order_id,'third_distribution_type'=>$third_distribution_type))->find();
		$thirth_data['addtime'] = time();
		if(!empty($thirth_query)){
			M('lionfish_comshop_orderdistribution_thirth_query')->where(array('id'=>$thirth_query['id']))->save($thirth_data);
		}else{
			M('lionfish_comshop_orderdistribution_thirth_query')->add($thirth_data);
		}
	}
	...
	public function getOrderStoreAddress($order_info){
		...
		//storename
		$lionfish_comshop_supply = M('lionfish_comshop_supply')->where( array('id' => $store_id) )->find();	
		...
		$store_data['begin_username'] = $lionfish_comshop_supply['storename'];
		return $store_data;
	}
}