...
class PingoodsModel {
	...
	**/
	...
	public function get_weshare_image($goods_id , $member_id)
	{
		...
		$goods_info = M('lionfish_comshop_goods')->field('goodsname,price,productprice,sales,seller_count,total,type')->where( array('id' => $goods_id) )->find();
	...
	public function _get_index_wxqrcode($member_id,$community_id,$suffix="png")
	{
		...
		//qrcode
		$jssdk = new \Lib\Weixin\Jssdk( $weixin_config['appid'] ,$weixin_config['appscert'] );
		$weqrcode = $jssdk->getAllWeQrcode('lionfish_comshop/pages/index/index',$community_id .'_'. $member_id,$suffix=="png",$qrcode_arr);
		//line_color
	...
	**/
	public function get_goods_price($goods_id,$member_id = 0)
	{
		...
		$goods_info = M('lionfish_comshop_goods')->field('is_take_vipcard,price as danprice,type,card_price')->where( array('id' => $goods_id))->find();
		...
		if($goods_info['type'] =='pin')
		{
			...
			if(!empty($pin_goods_info))
			{
				...
				$max_option_pinprice_arr = M('lionfish_comshop_goods_option_item_value')->field('pinprice as pin_price')->where( array('goods_id' => $goods_id) )->order('pinprice desc')->find();
		...
		$goods_common = M('lionfish_comshop_good_common')->field('is_mb_level_buy,has_mb_level_buy,mb_level_buy_list')->where( array('goods_id' => $goods_id ) )->find();
