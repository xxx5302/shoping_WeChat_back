...
function all_qiniu() {
    ...
    //qiniu_accesskey
    $accessKey_arr = M('lionfish_comshop_config')->where(array(
        'name' => 'qiniu_accesskey'
    ))->find();
    $secretKey_arr = M('lionfish_comshop_config')->where(array(
        'name' => 'qiniu_secretkey'
    ))->find();
    $bucket_arr = M('lionfish_comshop_config')->where(array(
        'name' => 'qiniu_bucket'
    ))->find();
...
function tpl_form_field_image2($name, $value = '', $default = '', $options = array()) {
    ...
    if (!empty($value)) {
        ...
        $val = $val ? $val : '/static/images/default-pic.jpg';
    }
...
function pagination2($total, $pageIndex, $pageSize = 15, $url = '', $context = array(
    ...
    $html = '<div><ul class="pagination pagination-centered"><li><span class="nobg">共' . $total . '条记录</span><span class="nobg">共' . $pageNum . '页</span></li></ul>';
    if (!empty($total)) {
        $pdata['tcount'] = $total;
        $pdata['tpage'] = empty($pageSize) || ($pageSize < 0) ? 1 : ceil($total / $pageSize);
        if (1 < $pdata['tpage']) {
...
function resize($filename, $width, $height) {
    ...
    if (!is_file($image_dir . $new_image) || (filectime($image_dir . $old_image) > filectime($image_dir . $new_image))) {
        ...
        //
        $attachment_type_arr = M('lionfish_comshop_config')->where(array(
...
if (!function_exists('load_make_plug')) {
    function load_make_plug($name = '')
    {
        ...
        $model = ROOT_PATH."/addons/lionfish_comshop_plugin_make/" . 'model/' . strtolower($name) . 'ModelClass.php';
        if (!is_file($model)) {
...
{
    $data['type'] = $type;
    $data['s_id'] = $_SESSION['oscshop_seller_s']['seller_auth']['uid'];
